...
uint8_t Adafruit_NFCShield_I2C::mifareclassic_WriteNDEFString (uint8_t sectorNumber, const char * str)
{
  // Figure out how long the string is
  uint8_t len = strlen(str);
  
  // Make sure we're within a 1K limit for the sector number
  if ((sectorNumber < 4) || (sectorNumber > 64))
    return 0;
  ...
  // Setup the sector buffer (w/pre-formatted TLV wrapper and NDEF message)
  uint8_t sectorbuffer1[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  if (len <= 16)
  {
    // Unlikely we'll get a url this short, but why not ...
    memcpy (sectorbuffer1, str, len);
  }
  
  
  // Now write all three blocks back to the card
  if (!(mifareclassic_WriteDataBlock (sectorNumber, sectorbuffer1)))
    return 0;
